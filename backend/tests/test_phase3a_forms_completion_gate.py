...
def test_forms_required_keys_gate_job_completion_and_invoice_release(client):
    ...
    # Create customer+job via Phase 4 flow (reuse structure to keep this test focused).
    ...
    quote_res = client.post(
        ...
        json={
            ...
            "currency": "GBP",
            "notes": "Phase3a forms test quote",
            "items": [
    ...
    # Configure required form keys for completion gate.
    req_res = client.post(
        f"/jobs/{job_id}/forms/CP12/requirements",
        headers=_auth_headers(admin_token),
        json={"required_keys": ["reading_1"]},
    )
    assert req_res.status_code == 200, req_res.text

    bundle = client.get(
        f"/jobs/{job_id}/completion-requirements",
        headers=_auth_headers(engineer_token),
    )
    assert bundle.status_code == 200, bundle.text
    b = bundle.json()
    assert b["job_id"] == job_id
    assert len(b["form_requirements"]) >= 1
    assert b["form_requirements"][0]["form_key"] == "CP12"

    # Engineer punches in/out -> should block completion due to missing form data.
    ...
    job = _find_job(jobs_res.json(), job_id)
    assert job is not None
    assert job["status"] == "completion_blocked_forms"

    # Compliance can still generate (separate concern).
    ...
    # Invoice release is blocked because job isn't fully completed.
    ...
    # Engineer submits the required reading -> job should finalize and invoice generation should work.
    submit_res = client.post(
        f"/jobs/{job_id}/forms/CP12/submit",
        headers=_auth_headers(engineer_token),
        json={"data": {"reading_1": "123"}},
    )
